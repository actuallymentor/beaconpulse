...
export async function sign_message( claimed_message, claimed_signatory ) {
	...
	// If claimed message is json, stringify it
	if( typeof claimed_message == 'object' ) claimed_message = JSON.stringify( claimed_message )
...
export function useIsConnected() {
	...
	log( `Initial connected state: `, window.ethereum?.isConnected() )
	const [ isConnected, setIsConnected ] = useState( window.ethereum?.isConnected() )

	// Listen to disconnects
	useEffect( f => setListenerAndReturnUnlistener( window.ethereum, 'disconnect', f => {
		log( `Connected sate: `, false )
		setIsConnected( false )
	} ), [] )

	// Listen to connects
	useEffect( f => setListenerAndReturnUnlistener( window.ethereum, 'connect', f => {
		log( `Connected sate: `, true )
		setIsConnected( true )
	} ), [] )
